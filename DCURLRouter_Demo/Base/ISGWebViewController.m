...
- (void)viewDidLoad {
    ...
    NSString *urlString = self.params[@"urlStr"];
...
-(BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    //特殊情况，有可能出现突然跳进登录界面的html，所以需要对其进行调整，那么需要先登录在进行跳转界面
    NSString *tmpStr = [[request URL] absoluteString];
    NSArray * judgeWhethNeedLogInArray = [tmpStr componentsSeparatedByString:@"?"];
...
-(UIWebView*)webView{
    if (!_webView) {
        _webView = [[UIWebView alloc] initWithFrame:CGRectMake(0, 0, 430, 430-64)];
...
#pragma mark - private method---根据返回的
...
{   NSString *host = [URL host];
    ...
    if ([host isEqualToString:@"shareClick"]) {
        ...
        [self share:URL];
    } else if ([host isEqualToString:@"pay_event"]) {
        [self pay:URL];
    }
}
...
- (void)share:(NSURL *)URL
{
    ...
    self.shareTitle = [tempDic objectForKey:@"shareTitle"];
    self.shareContent = [tempDic objectForKey:@"shareContent"];
    self.shareUrl = [tempDic objectForKey:@"shareUrl"];
    self.shareImageUrl = [tempDic objectForKey:@"shareImageUrl"];
    // 在这里执行分享的操作
//    [self shareGoodsMessage];
    // 将分享结果返回给js
}
...
//    NSString *payTitle = [NSString stringWithFormat:@"WISE-订单编号:%@",orderNum];
//    NSDictionary *dic = @{@"title":payTitle,
//                          @"total_fee":payAmount,
//                          @"bill_no":orderNum,
//                          @"notify_url":@"https://www.ketongtx.com/api/payment/wxpay/notify_url.aspx",
//                          @"bill_timeout":@"360",
//                          };
//    NSInteger payType = PayChannelAliApp;//PayChannelAliApppayType
//    if ([payTypeJudge isEqualToString:@"1"]) {
//        payType = PayChannelWxApp;
//    }
//    else if ([payTypeJudge isEqualToString:@"2"]) {
//        payType = PayChannelAliApp;
//    }
//    NSString *paySource = @"";//没有的话或者是0代表是从原生的跳转的--否则代表是从芝麻城跳转的--------
//    BeeCloudPayManager *manager = [BeeCloudPayManager payManager];
//    [manager initializeBeeCloudIDWithSource:paySource];
//    /**
//     *  支付渠道，PayChannelWxApp,PayChannelAliApp,PayChannelUnApp,PayChannelBaiduApp
//     */
//    [manager doPay:payType info:dic];
}
